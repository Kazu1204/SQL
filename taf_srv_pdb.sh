...
echo "srvctl add service -d ${TAF_DB_NAME} -s ${TAF_SRV_NAME} -g ${TAF_SRVPOOL_NAME} -pdb ${TAF_PDB_NAME} -e SELECT -m BASIC -w 1 -z 5"
srvctl add service -d ${TAF_DB_NAME} -s ${TAF_SRV_NAME} -g ${TAF_SRVPOOL_NAME} -pdb ${TAF_PDB_NAME} -e SELECT -m BASIC -w 1 -z 5
...
TAF_CONNECT="conn taf_user/taf_user@SCANNAME/${TAF_SRV_NAME}"
